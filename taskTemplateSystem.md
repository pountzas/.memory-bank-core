...
| `memory-bank/mechanisms/`           | Mechanisms that enforce workflow guardrails (e.g. cwd safety, git)    |
...
5. Instantiate Task
   ...
   - Ensure `repo_path` is set to the repository root (absolute path). If missing/unknown, STOP and ask the user.
...
* **Terminal safety**: Before running any terminal command, set and verify CWD using `{repo_path}`:
...
| `llm_mistake_type`   | enum   | {misinterpretation, missing_step, wrong_output, wrong_cwd, git_workflow_violation, esm_transform_missing, missing_act_wrapper, other} |
...
* Wrong CWD: If any command runs outside `{repo_path}` or a template hardcodes a different project path → add/strengthen explicit CWD verification step and remove hardcoded paths from templates/examples.
...
5. Execute next `todo` task (Terminal rule: before any terminal command, `Set-Location -Path "{repo_path}"; (Get-Location).Path`).
