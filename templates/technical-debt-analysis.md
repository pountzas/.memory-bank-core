---
template_id: technical-debt-analysis
title: Tech Debt Analyzer - Claude Skill Integration
intent: Leverage Claude AI skill for tech-debt-analyzer to enhance development workflow and deliver specialized expertise
...
steps: |
  0. Verify current branch location and create feature branch if necessary: git branch --show-current, create feature/claude-tech-debt-analyzer if on dev (MANDATORY - before any work)
...
dependencies: []
version: 1.0.0
notes: This template leverages the specialized Claude AI skill "tech-debt-analyzer" for enhanced development capabilities. Ensure skill prerequisites are met before execution. CRITICAL - Create feature branch BEFORE any work begins. Never commit directly to dev. Always verify workspace directory location. ALWAYS use PowerShell for terminal commands. RIGHT BEFORE git add: Test skill outputs thoroughly.
learning_tags: [claude-skill, tech-debt-analyzer, automation, ai-enhancement]
changelog:
  - "2025-12-19: Initial Claude skill integration template for tech-debt-analyzer."
---

## Claude Skill: Tech Debt Analyzer
...
- `DEBT_REGISTER_TEMPLATE.md` (file)

### Scripts
- `analyze_dependencies.py` (file)
- `detect_code_smells.py` (file)

### References
- `debt_categories.md` (file)
...
status: todo
lane: ai-enhancement
locks: ["tech-debt-analyzer-related-files"]
depends_on: []
branch: feature/claude-tech-debt-analyzer
timebox_minutes: 60
```
