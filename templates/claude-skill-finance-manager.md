---
template_id: claude-skill-finance-manager
title: Finance Manager - Claude Skill Integration
intent: Leverage Claude AI skill for finance-manager to enhance development workflow and deliver specialized expertise
...
steps: |
  0. Verify current branch location and create feature branch if necessary: git branch --show-current, create feature/claude-finance-manager if on dev (MANDATORY - before any work)
...
### Scripts
- `analyze_finances.py` (file)
- `extract_pdf_data.py` (file)
- `generate_report.py` (file)

### References
- `financial_frameworks.md` (file)
...
status: todo
lane: ai-enhancement
locks: ["finance-manager-related-files"]
depends_on: []
branch: feature/claude-finance-manager
timebox_minutes: 60
```
