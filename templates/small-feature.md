...
steps: |
  ...
  2. Verify workspace location (MANDATORY): Set-Location -Path "{repo_path}"; (Get-Location).Path
  ...
  9. Document any mistakes made during implementation (MANDATORY if any occurred)
  10. Push feature branch and create PR targeting dev
acceptance_criteria: |
...
dependencies: []
version: 1.1.1
notes: Follow TDD cycle (RED->GREEN->REFACTOR). Keep scope tight; avoid refactors unless essential. Write tests before implementation. CRITICAL - Create feature branch BEFORE any work begins. Never commit directly to dev. MANDATORY - Document any mistakes in progress.md with mistake_description, impact, correct_approach, and prevention_measures. Always verify workspace directory location using {repo_path} (never hardcode absolute paths). ALWAYS use PowerShell for terminal commands. RIGHT BEFORE git add: Use MCP Chrome DevTools to verify app functionality and absence of errors.
