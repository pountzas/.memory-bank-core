...
steps: |
  ...
  1. Verify workspace location (MANDATORY): Set-Location -Path "{repo_path}"; (Get-Location).Path
...
dependencies: []
version: 1.0.2
notes: Keep changes minimal and test-driven. CRITICAL - Create fix branch BEFORE any work begins. Never commit directly to dev. Always verify workspace directory location using {repo_path} (never hardcode absolute paths). ALWAYS use PowerShell for terminal commands. RIGHT BEFORE git add: Use MCP Chrome DevTools to verify app functionality and absence of errors.
